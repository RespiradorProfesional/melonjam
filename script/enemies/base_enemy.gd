...
var death= false
@onready var attack_stream: AudioStreamPlayer3D = $attack
@onready var death_stream: AudioStreamPlayer3D = $death
@onready var attacked_stream: AudioStreamPlayer3D = $attacked
...
func _on_area_3d_body_entered(body: Node3D) -> void:
	if body is Player:
		body.hit(global_position, damamge, knockback_strength)
...
func damaged(from_position: Vector3,damage,knockback_strenght,knockback:bool=false):
	...
	if hp - damage <=0:
		...
		var elements = ["nothing", "fire", "wind", "water"]
		GlobalDataUser.add_elemnt( elements[randi() % elements.size()])
		death= true
		
	else:
		attacked_stream.play()
		hp -= damage
